...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    async function fetchOfferListings(){
      try {
        const res = await fetch('https://realestateportal-backend.onrender.com/api/listing/get?offer=true&limit=4');
          ...
          saleListings && saleListings.length > 0 && (
            <div className=''>
              <div className='my-3'>
                ...
                <Link to={'/search?type=sell'} className='text-sm text-blue-800 hover:underline'>Show more places for sale</Link>
              </div>
              <div className='flex flex-wrap gap-4'>
                {
                  offerListings.map((listing)=>(
